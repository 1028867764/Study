...
// Example data structure (you can replace this with your actual data)
...
void main() {
  final outputDir = Directory('markdown_files');
  if (!outputDir.existsSync()) {
    outputDir.createSync();
  }

  // Loop through each product and create markdown files
    ...
    for (var parentGroup in category.parentProductGroups) {
      for (var parentProduct in parentGroup.parentProducts) {
        for (var product in parentProduct.childProducts) {
          ...
          final productNameFirstValue = product.name[0];

          // Create the markdown file name
          final markdownFileName =
              '${productId}_markdown_1_${productNameFirstValue}.md';
          final markdownFilePath = '${outputDir.path}/$markdownFileName';

          // Create the markdown file and write the content
          final file = File(markdownFilePath);
          file.writeAsStringSync('# Product Name: $productNameFirstValue\n\n');
        }
      }
    }
  }
